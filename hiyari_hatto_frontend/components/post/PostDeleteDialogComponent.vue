<template>
  <v-btn rounded color="grey" @click="showDialog">投稿削除</v-btn>
...
export default defineComponent({
  ...
  setup() {
    ...
    const successDeleteDisplay = () => {
      root.$toast.success("投稿を削除しました");
    };

    const errorDeleteDisplay = () => {
      root.$toast.error("投稿の削除に失敗しました");
    };

    async function onSubmit() {
      const { deletePost } = usePostFunction();
      try {
        const result = await deletePost(postDetail.id);
        if (result) {
